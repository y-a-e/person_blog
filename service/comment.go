...
func (commentService *CommentService) CommentDelete(c *gin.Context, req request.CommentDelete) error {
	...
	return global.DB.Transaction(func(tx *gorm.DB) error {
		for _, id := range req.IDs {
			...
			if comment.UserUUID != userUUID && comment.User.RoleID != userRoleID {
				return errors.New("you do not have permission to delete this comment")
			}

			if err := commentService.DeleteCommentAndChild(tx, id); err != nil {
				return err
			}
		}
		return nil
...
func (commentService *CommentService) CommentInfo(uuid uuid.UUID) ([]database.Comment, error) {
	...
	// 查询个人全部评论
	if err := global.DB.Order("id desc").Where("user_uuid", uuid).Preload("User", userPreload).Find(&rawComments).Error; err != nil {
		return nil, err
	}
...
func (commentService *CommentService) CommentInfoByArticleID(req request.CommentInfoByArticleID) ([]database.Comment, error) {
	...
	// 查询一级评论
	if err := global.DB.Where("article_id = ? AND p_id IS NULL", req.ArticleID).Preload("User", userPreload).Find(&comments).Error; err != nil {
		return nil, err
	}
...
func (commentService *CommentService) CommentNew() ([]database.Comment, error) {
	...
	// 查询最新5条评论
	if err := global.DB.Order("id desc").Limit(5).Preload("User", userPreload).Find(&comments).Error; err != nil {
		return nil, err
	}
