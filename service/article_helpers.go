...
func (articleService *ArticleService) UpdateCategoryCount(tx *gorm.DB, oldCategory, newCategory string) error {
	...
	if newCategory != "" {
		...
		err := tx.Where("category = ?", newCategory).First(&newArticleCategory).Error
...
func (articleService *ArticleService) UpdateTagsCount(tx *gorm.DB, oldTags, newTags []string) error {
	...
	for _, addedTag := range addedTags {
		...
		err := tx.Where("tag = ?", addedTag).First(&t).Error
		if errors.Is(err, gorm.ErrRecordNotFound) {
	...
	for _, removedTag := range removedTags {
		...
		if t.Number == 1 {
			if err := tx.Delete(&t).Error; err != nil { // GORM 没有正确构建 WHERE 条件(自动拼接主键) 已解决
				// if err := tx.Where("tag = ?", removedTag).Delete(&t).Error; err != nil {	// 找到问题所在，database/article_tag.go的tag标签符号编写错误
				return err
			}
		}
	}
